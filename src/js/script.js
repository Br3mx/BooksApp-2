...

{
  ...
  class BooksList {
    ...
    initActions() {
      const thisBooksList = this;
      thisBooksList.dom.booksList.addEventListener('dblclick',
        function (event) {
    ...
    filterBooks() {
      ...
      for (let book of thisBooksList.data) {
        ...
        const filtereBook = document.querySelector(
          '.book__image[data-id="' + book.id + '"]'
        );

        if (shouldBeHidden) {
          filtereBook.classList.add('hidden');
        } else {
          filtereBook.classList.remove('hidden');
        }
